...
const roleEntitlementMember = "member"
...
func roleResource(role *rbacv1.Role) (*v2.Resource, error) {
	...
	if role.Labels != nil {
		profile["labels"] = k8s.StringMapToAnyMap(role.Labels)
	}
	if role.Annotations != nil {
		profile["annotations"] = k8s.StringMapToAnyMap(role.Annotations)
	}

	// Get parent namespace resource ID
	parentID, err := k8s.NamespaceResourceID(role.Namespace)
	if err != nil {
	...
	for _, binding := range matchingBindings {
		...
		for _, subject := range binding.Subjects {
			if subject.Kind == k8s.SubjectKindServiceAccount {
				// This are Cluster's local service accounts, not Azure.
